<template>
  <div class="consult-detail-page" v-if="item">
    ...
    <div class="detail-action van-hairline--top" v-if="item.status === OrderType.ConsultPay">
      ...
      <van-button type="default" round :loading="loadig" @click="cancelConsultOrder(item!)">
        取消问诊
      </van-button>
    ...
    <div class="detail-action van-hairline--top" v-if="item.status === OrderType.ConsultComplete">
      ...
      />
      <van-button type="default" round :to="`/room?orderId=${item.id}`">聊天记录</van-button>
    ...
    <div class="detail-action van-hairline--top" v-if="item.status === OrderType.ConsultCancel">
      <van-button type="default" round @click="deleteConsultOrder(item!)" :loading="deleteLoading">
        删除订单
      </van-button>
...
// hook - 删除订单
const { deleteConsultOrder, loading: deleteLoading } = useDeleteOrder(() => {
...
onMounted(async () => {
  const { data } = await getConsultOrderDetailAPI(route.params.id as string)
  item.value = data
})
...
const onCopy = () => {
  if (!isSupported.value) return showToast('不支持')
