...
<script>
import { reqGetUserDetailById, reqGetPersonalDetail, getJobDetail } from '@/api/employees'
export default {
  name: 'Print',
  data() {
    return {
  ...
  methods: {
    async getPersonalDetail() {
      const userInfo = await reqGetUserDetailById(this.userId) // 获取个人基本信息(顶部)
      const detail = await reqGetPersonalDetail(this.userId) // 获取个人基本信息(底部)
