      ...
      <el-row class="inline-info">
        ...
        <el-col :span="12">
          <el-form-item label="入职时间">
            <el-date-picker v-model="userInfo.timeOfEntry" type="date" class="inputW" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 姓名 部门 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="最高学历">
          <el-select v-model="formData.theHighestDegreeOfEducation" class="inputW2">
            <el-option
              v-for="item in EmployeeEnum.highestDegree" :key="item.value" :label="item.label"
              :value="item.value"
            />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <ImageUpload ref="moreImgUpload" :limit="2" />
        </el-form-item>
        <el-form-item label="国家/地区">
          <el-select v-model="formData.nationalArea" class="inputW2">
            <el-option
              v-for="item in EmployeeEnum.isOverseas" :key="item.value" :label="item.label"
              :value="item.value"
            />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="婚姻状况">
          <el-select v-model="formData.maritalStatus" class="inputW2">
            <el-option
              v-for="item in EmployeeEnum.maritaStatus" :key="item.value" :label="item.label"
              :value="item.value"
            />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="星座">
          <el-select v-model="formData.constellation" class="inputW2">
            <el-option
              v-for="item in EmployeeEnum.constellation" :key="item.value" :label="item.label"
              :value="item.value"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="血型">
          <el-select v-model="formData.bloodType" class="inputW2">
            <el-option
              v-for="item in EmployeeEnum.bloodType" :key="item.value" :label="item.label"
              :value="item.value"
            />
          </el-select>
        </el-form-item>
      ...
      <div class="block">
        ...
        <el-form-item label="联系手机">
          <el-input
            v-model="formData.contactTheMobilePhone" placeholder="11位字符" maxlength="11" class="inputW"
      ...
      <div class="block">
        ...
        <el-form-item label="学历类型">
          <el-select v-model="formData.educationalType" placeholder="请选择">
            <el-option
              v-for="item in EmployeeEnum.educationType" :key="item.value" :label="item.label"
              :value="item.value"
            />
          </el-select>
        </el-form-item>
...
import { saveUserDetailById, reqGetPersonalDetail, reqUpdatePersonal } from '@/api/employees'
...
export default {
  ...
  methods: {
    ...
    async saveUser() {
      if (!this.$refs.staffRef.isAllUploadSuccess) {
        this.$message.error('图片正在上传中，请稍后')
        return
      }
      if (!this.$refs.staffRef.fileList[0]?.url) {
        this.$message.error('请上传图片')
        return
      }
      await saveUserDetailById({ ...this.userInfo, staffPhoto: this.$refs.staffRef.fileList[0].url })
      this.$message.success('保存成功')
    },

    async getPersonalDetail() {
      const res = await reqGetPersonalDetail(this.userId) // 获取员工数据
      this.formData = res
      // 头像回显
      this.$refs.moreImgUpload.fileList.push({ url: res.staffPhoto })
    },
    async savePersonal() {
      // 在保存用户头像之前，判断用户是否上传了头像和是否有头像
      if (!this.$refs.moreImgUpload.isAllUploadSuccess) {
        this.$message.error('图片正在上传中，请稍后')
        return
      }
      if (this.$refs.moreImgUpload.fileList.length < 0) {
        this.$message.error('请上传图片')
        return
      }
      await reqUpdatePersonal({ ...this.formData, userId: this.userId, staffPhoto: this.$refs.moreImgUpload.fileList[0].url })
      this.$message.success('保存成功')
    }
  }
}
